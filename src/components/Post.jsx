...
const Post = () => {
  ...
  useEffect(() => {
    Promise.all([
      fetch(`https://fake-json-api.mock.beeceptor.com/users`).then((res) =>
        res.json()
      ),
      ...
      .then(([users, posts, data]) => {
        const merged = posts.map((post) => {
          const user = users.find((u) => u.id === post.userId);
          const dataset = data.users.find((d) => d.id === post.id);
          return {
            ...post,
            user,
            dataset,
      ...
      {feed.map((item, i) => (
        ...
        >
            ...
            <div className="h-fit w-full flex flex-col pt-2 pb-2 relative">
              <div className="h-fit w-full">
                <span className="text-(--current-color) font-bold pr-2 hover:underline">
                  {item.user?.name.trim().split(/\s+/).slice(0, 2).join(" ")}
                </span>
          ...
          <div className="h-fit w-full flex flex-col justify-center items-start pl-[10%]">
            <div className="sm:h-125 h-100 w-[75%] flex items-center border border-(--border-color) rounded-3xl">
